...
- Over Streamable HTTP MCP, graph tools (`gno_impact`, `gno_backlinks`, `gno_graph*`) check egress policy on their collection argument, or on every collection when none is given, because a link can resolve into another collection.
