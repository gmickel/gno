...
- MCP on the `full` profile now registers 37 read-only tools (adds `gno_sessions_status`) and 58 with `--enable-write` (adds `gno_request_status`, `gno_sessions_status`, and `gno_sessions_import`).
