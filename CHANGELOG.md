...
- Wiki links now resolve across collections that share one Obsidian vault. If you index a vault as several collections (for example one per top-level folder), a plain `[[Note]]` finds its target in any of them, and vault-root path links such as `[[Spaces/AI/Agents/_index]]` resolve to the one right file instead of showing up as unresolved or ambiguous. GNO detects the vault from its `.obsidian` folder; set `workspaceRoot` on a collection to join a workspace without one (Logseq, Foam, Dendron, plain folders) or `workspaceRoot: false` to keep a collection's links to itself. Ranking follows Obsidian: exact path first, then a file in the linking note's folder, then the file with the fewest folders above it; files that stay tied are reported by `gno audit links` and create no graph link. Backlinks, `gno links`, `gno impact`, graph neighbours in search, ask and Context Capsules, graph export and `gno audit links` all use the same resolution. See [Link workspaces](docs/CONFIGURATION.md#link-workspaces).
- `gno impact` (and REST `/api/impact`, MCP `gno_impact`) accepts collections to limit the traversal.
- `gno status` and `gno collection list` show each collection's link workspace and whether it was detected or configured.

### Changed

- **Upgrade note:** link workspaces are on by default. The first `gno update` after upgrading rebuilds the link graph (and says so). Resolving a link never widens what you get back: a search, answer, Context Capsule, backlink list or impact run scoped to some collections still returns documents from those collections only, and collection egress policies apply unchanged. Links inside a vault can still resolve differently than before, for example `[[Roadmap]]` now prefers a file named `Roadmap.md` over a note whose title is "Roadmap". Set `workspaceRoot: false` on a collection to keep the previous behavior for it.
- `gno audit links` findings now say what kind of reference failed, whether it is unresolved or ambiguous, and whether it crossed collections; ambiguous vault links list the tied candidates (candidates outside a collection-scoped audit are counted, not named). A document linked only from another collection of its vault is no longer reported as an orphan.
- Over Streamable HTTP MCP, graph tools (`gno_impact`, `gno_backlinks`, `gno_graph*`) check egress policy on their collection argument (every collection when none is given, because a link can resolve into another collection) and always on the collection of the document they are asked about. Naming a `local_only` document while scoping the call to a `remote` collection is refused. `gno_similar` with `crossCollection: true` is checked against every collection.
...
- Security: over Streamable HTTP MCP, a remote call with a blank `collection` (for example `gno_search` with `collection: ""`) passed the collection egress check as if it were scoped to no collection, while the tool treated the blank value as "all collections". Remote callers could receive results from `local_only` collections. A blank collection is now treated as unscoped, so every collection's egress policy applies.
...
- Windows: a first request-ID write interrupted before the private ledger directory was secured (for example by a slow PowerShell start) no longer leaves every later request-ID write refused. An empty ledger directory is now secured like a new one, and the permission check allows 30 seconds for PowerShell to start.
...
- A genuine fork is never silent: when a runtime cannot reproduce the stored vectors, `gno embed` states that it would build a separate partition, the full chunk count and a measured estimate, and requires confirmation (interactive prompt or `--new-partition`; `--yes` alone does not confirm). A confirmed partition is reused by later compatible versions of that runtime, and changing a vector-defining setting for a model that already has vectors needs the same confirmation. Queries from that runtime use lexical retrieval only, with a `vector_runtime_incompatible` warning.
- `gno status`, `gno doctor`, REST `/api/status`, MCP `gno_status` and the SDK report embedding counts against the partition the calling runtime's queries read (same selection rule as retrieval, `vectorRuntime`) and list every vector partition (`vectorPartitions`) with state, chunk count, provenance such as `CUDA, Bun 1.4.2`, and the runtimes that read or cannot read it, so an incomplete shadow partition no longer reads as lost embeddings.
...
- `gno status`, `gno serve --status`, and `gno daemon --status` report background issues: a failing, parked, or overrunning background embed pass, or a detached resident that does not answer its status request. Each resident query is bounded to 500ms, so status never waits on a hung resident.
- `gno vec drop <partition>` removes an abandoned shadow vector partition (legacy shadows included) the calling runtime does not read; status prints its hint for exactly those partitions, and every active partition, legacy included, is refused. `gno vsearch` from a runtime without a usable partition says vector search is unavailable and points to `gno query` / `gno search` or `gno embed --new-partition`.
...
- Opt-in session automation: `gno sessions automation` profiles name registered sources and switch on a Claude Code SessionEnd hook (one owned settings entry, verified with Claude Code 2.1.280) and/or an elapsed-cadence schedule separately. Triggers only mark a profile pending durably; `gno daemon` on the archive drains pending work at startup and every 30 seconds through the manual importer, and `gno sessions automation run` runs a profile now. Status on every surface shows daemon availability, pending/running/partial/failed state, last success, next due time, and recovery actions; missed intervals coalesce, failed runs back off, and pause/remove only touch GNO's own hook entry. Also available as MCP `gno_sessions_automation_run`, REST `/api/sessions/automation/*`, SDK `runSessionsAutomation()`, and an Automation panel on `/sessions`. Install, upgrade, repair, and restart never enable automation. See [Automation](docs/SESSIONS.md#automation-opt-in).
- Opt-in request IDs for retry-safe writes: `--request-id` on `gno capture` and `gno remember`, `requestId` on MCP `gno_capture` / `gno_remember`, REST `POST /api/capture`, `POST /api/memory/remember`, and `PUT /api/docs/:id`, and SDK `client.capture()` / `client.remember()`. Retrying with the same ID replays or finishes the write instead of writing twice. Requests are recorded in a private per-index ledger that index maintenance and `gno reset` keep.
...
- `gno capture` and `client.capture()` now plan and write under the shared write lease, so a busy lease returns the busy error. Without a request ID their sync outcome is unchanged (`sync.status: "failed"` or `"skipped"`); with one, a sync failure is an error and the request stays pending until retried.
- MCP on the `full` profile now registers 37 read-only tools (adds `gno_sessions_status`) and 59 with `--enable-write` (adds `gno_request_status`, `gno_sessions_status`, `gno_sessions_import`, and `gno_sessions_automation_run`).
...
- Wait for pending ClawHub publications before verifying public file hashes, with a verification-only retry that avoids duplicate uploads.
