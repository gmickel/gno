...
| 3 recall budget | Mean recall@5 (quality queries); cite validity; facts ≤ 8, tokens ≤ 512 | ≥ 0.8; 1.0; 1.0                                      |
| 4 fence         | Receipted replays + `gno://` derived origins rejected                   | 1.0 (paraphrase leak-through reported, not asserted) |
| 5 scopes        | Foreign-scope facts returned or reused on write                         | 0                                                    |
...
pin is stale. Every file carries `suite` and `description`; the rest is:
...
| `recall.json`           | `facts[]` (`id`, `text`) and `queries[]` (`id`, `query`, `relevant[]`, optional `kind: "budget"` for more relevant facts than the budget admits)                                                                                                                                                                                                             |
...
| `scopes.json`           | `facts[]` with `scopes[]`, `reads[]` (`query`, `scopes[]`), `writes[]` (`text`, `scopes[]`, `expect`)                                                                                                                                                                                                                                                        |
