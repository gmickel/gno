...
import { runGeneralEmbeddingBenchmark } from "../evals/helpers/general-embedding-benchmark";
...
function parseArgs(args: string[]): CliOptions {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    switch (arg) {
      ...
      default:
        throw new Error(
          `Unknown argument: ${arg}. Use --candidate <id> or --model <uri> [--label <name>] [--cache-dir <path>] [--db-path <path>] [--write] [--out <path>] [--dry-run]`
        );
    }
  }
  return options;
...
function toMarkdown(
  summary: Awaited<ReturnType<typeof runGeneralEmbeddingBenchmark>>
): string {
  const rows = summary.cases
    .map(
      (item) =>
        `| ${item.id} | ${item.caseSet} | ${item.queryLanguage} | ${item.vectorMetrics.ndcgAt10.toFixed(3)} | ${item.hybridMetrics.ndcgAt10.toFixed(3)} | ${item.vectorLatencyMs.toFixed(1)}ms | ${item.hybridLatencyMs.toFixed(1)}ms |`
    )
    .join("\n");

  return `# General Embedding Benchmark
...
| --- | ---: | ---: |
| same-language | ${summary.bySet["same-language"]?.vector.ndcgAt10.toFixed(3) ?? "0.000"} | ${summary.bySet["same-language"]?.hybrid.ndcgAt10.toFixed(3) ?? "0.000"} |
| cross-language | ${summary.bySet["cross-language"]?.vector.ndcgAt10.toFixed(3) ?? "0.000"} | ${summary.bySet["cross-language"]?.hybrid.ndcgAt10.toFixed(3) ?? "0.000"} |
...
const summary = await runGeneralEmbeddingBenchmark({
  embedModel: model,
  label: options.label ?? candidate?.label ?? model,
...
if (options.write) {
  ...
  const candidateSlug = slugify(
    options.candidate ?? options.label ?? candidate?.label ?? model
  );
  await Bun.write(join(outDir, `${stamp}-${candidateSlug}.json`), json);
  await Bun.write(
    join(outDir, `${stamp}-${candidateSlug}.md`),
    `${toMarkdown(summary)}\n`
  );
