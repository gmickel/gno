...
- Database schema version participates in saved Capsule/index fingerprints and verifier inputs. A trial migration changed those inputs and failed the existing frozen Ask parity test, even with unchanged chunks. Use existing schema_meta keys for the index target and each mirror's applied policy/provenance; add no migration. Legacy missing metadata means defaults without backfill. Empty applied content gets an explicit marker; orphan cleanup removes its metadata when content is collected. Source: src/core/context-evidence.ts, test/pipeline/ask-hydration.test.ts, src/store/sqlite/chunking-policy.ts.
...
- Lexical BM25 indexes the complete canonical document, so changing chunk size alone can leave its ranking unchanged. Chunk boundaries affect semantic retrieval and evidence spans; experiments should distinguish these measures. Source: src/store/sqlite/adapter.ts (rebuildFtsForHash).
...
Run focused configuration, store, ingestion, status, and frozen Ask parity tests. Final gates are bun run lint:check, bun test, bun run docs:verify, bun run verify:clipper-package, bun run test:package, and the hybrid evaluation. Use Bun 1.4.2, matching CI. Validate the hosted documentation with its check, typecheck, test, build, and driven page checks before and after deployment.
