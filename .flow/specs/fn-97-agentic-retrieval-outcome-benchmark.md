...
The runner owns corpus setup, adapter lifecycle, the outer-agent loop, timeout/call budgets, and failure classification. `fixture-db.ts` canonicalizes fixture inputs once and produces one immutable fingerprinted corpus snapshot. During unmeasured preparation, each adapter may build its own native immutable index from exactly that snapshot: GNO MCP, lexical, and Capsule may share one GNO index; qmd builds its native index only inside temporary `QMD_CONFIG_DIR`/XDG/data roots. Every adapter records its index fingerprint and build observations. Cold and warm cohorts for one adapter reuse the identical prebuilt adapter-native index; cross-adapter index formats/content representations need not be identical. Pluggable adapters implement product-faithful GNO MCP, lexical-only, eval-only Capsule prototype, and qmd comparison. A normalized `TrajectoryReceipt` separates:
...
- Corpus snapshot canonicalization and adapter-native immutable index construction complete before both cold and warm cohorts and are reported only as preparation. Each adapter's index must derive from the same corpus-snapshot fingerprint, while its native index fingerprint/build observations are recorded separately. A `cold` trial starts a fresh adapter process against that adapter's prebuilt index and locked cached model; its first tool call is scored, with no readiness/warm-up call. Cold end-to-end timing starts before process startup and ends after the final envelope, including startup/model-load/tool/driver time but excluding preparation. A `warm` cohort preserves one adapter process, the identical adapter-native index, and loaded model after exactly one discarded readiness probe; it resets only task-visible agent state between scored trials. Warm end-to-end timing starts at the first scored agent step and ends after the final envelope, excluding preparation/startup/model load/readiness. Receipts split preparation/index-build, startup, model-load, tool, driver, and end-to-end timing for every adapter; unavailable components are `null` with a reason. Never compare unlike lifecycle states.
...
QMD_REPO=/Users/gordon/repos/qmd bun run eval:agentic -- --agent fixture --adapter qmd --lifecycle cold --write
