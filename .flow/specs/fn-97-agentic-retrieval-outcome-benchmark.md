...
The primary outer agent is a deterministic fixture agent with a pinned state machine, prompt, tool schemas, call budget, and stop policy. It consumes only the agent-visible brief and normalized tool results, then emits a structured `FinalEnvelope` containing `claims[]` (stable claim ID, text, and evidence citations), `abstained`, and `stopReason`. An optional cached-local-model lane uses one pinned prompt/model/tokenizer and exactly three paired trials per task and adapter; trial IDs/seeds and task order are shared across adapters. It is opt-in, requires no network/API key, and reports variance without replacing the deterministic promotion lane.

The runner owns corpus setup, adapter lifecycle, the outer-agent loop, timeout/call budgets, and failure classification. Pluggable adapters implement product-faithful GNO MCP, lexical-only, eval-only Capsule prototype, and qmd comparison. A normalized `TrajectoryReceipt` separates:

- `canonical`: schemas, task/adapter/trial IDs, normalized calls and arguments, returned evidence coordinates/hashes, model-visible UTF-8 byte counts, token counts when a pinned tokenizer is available, final envelope, stop/failure classification, and corpus/prompt/tool/model/runtime/config fingerprints.
- `observations`: monotonic timings, timestamps, process/resource measurements, temp paths, redacted diagnostics, and other volatile values excluded from canonical hashes.
...
- The qmd adapter requires `QMD_REPO` to name a dedicated checkout whose `HEAD` is exactly `e428df76bc0274d9e93eb7ca3e95673315c42e90`. Missing path, dirty checkout, revision mismatch, unavailable command, or unsupported contract is a fail-closed harness error. It never reads a global/PATH qmd install and never checks out, pulls, installs, or mutates the supplied repository.
...
- `cold` GNO MCP timing starts before spawning a new isolated server/store/model lifecycle for one task trial and ends after the final envelope; setup is included. `warm` timing uses one already-started server and indexed corpus for a task cohort, performs one unscored readiness call before the first measured trial, preserves the same process/model/store lifecycle, resets only task-visible agent state between trials, and excludes initial spawn/index/readiness. Never compare unlike lifecycle states.
...
- **R1:** At least 20 validated, leak-resistant tasks cover every agreed category; versioned public task fixtures and separate hidden oracles pin exact line/hash evidence semantics and abstention expectations.
- **R2:** One pinned deterministic fixture agent and runner execute identical agent-visible briefs against product-faithful GNO MCP, lexical-only, eval-only Capsule prototype, and fail-closed revision-pinned qmd adapters. The optional cached-local-model lane runs exactly three paired trials.
- **R3:** Receipts separate deterministic canonical data from volatile observations and record normalized calls, exact evidence spans/hashes, model-visible UTF-8 bytes, measured tokens when comparable, timing/lifecycle, structured claims/citations, stop/failure reason, and reproducibility fingerprints.
...
- **R6:** Capsule promotion is evaluated only on the same non-harness-failed task/trial pairs `P` for Capsule and current GNO in the same lifecycle. With `success_a(p) ∈ {0,1}`, retrieval calls `calls_a(p)`, and model-visible UTF-8 bytes `bytes_a(p)`: `ΣP success_capsule / |P| >= ΣP success_gno / |P|`; `1 - (ΣP calls_capsule / ΣP calls_gno) >= 0.25`; and `1 - (ΣP bytes_capsule / ΣP bytes_gno) >= 0.35`, with non-zero GNO denominators or a failed gate. Across substantive final claims in `P`, `linked_supported_claims_capsule / substantive_claims_capsule >= 0.95`, with a non-zero denominator; abstention-only tasks are scored by their completion predicates and do not fabricate claims. Every fixture-agent Capsule task also produces byte-identical canonical Capsule payload JSON and SHA-256 on two unchanged-input replays. Any missing pair, accuracy loss, threshold miss, denominator failure, or nondeterminism fails promotion rather than being averaged away.
...
1. `fn-97-agentic-retrieval-outcome-benchmark.1` — Define fixtures, schemas, receipts, hidden oracles, and deterministic scorers (**M**)
...
| R6 | Exact paired promotion formulas, claim linkage, and unchanged-input Capsule determinism. | fn-97-agentic-retrieval-outcome-benchmark.1, fn-97-agentic-retrieval-outcome-benchmark.4, fn-97-agentic-retrieval-outcome-benchmark.6 | — |
