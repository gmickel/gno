...
Compile from an identified Capsule plus explicit local goal/scope/output settings. Verify source hashes, selected evidence spans, runtime/tokenizer identity and current collection scope and egress policy before publication. Render deterministic source-grounded sections with exact source references and an explicit untrusted-evidence boundary. Do not transform retrieved instructions into active agent policy. The artifact includes its format version, Capsule identity, source verification identity, budget/token accounting and a deterministic content digest; a private sidecar beside the explicitly chosen output records its Capsule-file path, output digest and renderer settings. No hidden Capsule-body store or destination registry is introduced. Policy drift means the stored evidence egressLineage differs from current lineage resolved for the same collections, including removed or no-longer-eligible collections.

Determinism applies to the same verified Capsule and renderer settings. A fresh retrieval or generation may legitimately select different evidence and is not promised byte-identical output. Count the entire rendered artifact, including citations and framing, against the selected budget using the Capsule's recorded estimator. unicode_conservative Capsules remain supported without loading a model; only active_tokenizer Capsules require the recorded tokenizer to be available. If mandatory provenance/framing cannot fit, fail rather than emit an uncited fragment. Do not reword sources with an unmeasured generation step.
...
Each section already contains its citation and evidence boundary. Real packing calls the existing selectContextEvidence selector with the renderer's framing/citation overhead accounted for; it does not implement a second selector. It reuses Capsule evidence-selection rules and reports omitted required facets; fitting text alone does not establish sufficient context.

## API Contracts

<!-- scope: technical; source: inferred -->

Use `gno context compiled <compile|preview|check|refresh>` for the new workflow. Preserve the existing `gno context check` configuration-validation command and its exit/schema contract unchanged. Compile accepts an explicit Capsule JSON file, token budget and explicit output target; it does not overwrite hand-authored instructions. Check is read-only, exits distinctly for current/stale/invalid-unavailable states, and names whether sources, policy, tokenizer, renderer or output bytes changed. Check reads the artifact and its private sidecar plus the referenced Capsule file; callers may supply the Capsule file explicitly if relocated. Missing sidecar/Capsule is an actionable unverifiable state. Refresh rebuilds through the existing Capsule build path from the saved normalized retrieval request, verifies the new Capsule, then renders and atomically replaces only the owned artifact when its previous output digest matches; unexpected edits conflict instead of being discarded. It reports the fresh selection and new Capsule identity. If source/config/policy and rendered bytes are unchanged, refresh is a verified no-op. Persist the refreshed Capsule at the explicitly selected path; fail before publication when that path cannot be written.
...
| MCP        | Provide read-only bounded preview/check operations over caller-eligible inline Capsules and artifact bytes. No host file paths, destination registration or remote file-writing tool.                                                                                  |
| SDK / REST | Shared compiler/verifier contract. Local SDK supports explicit file writes/refresh; REST offers read-only preview/check over supplied bytes with caller scope enforced. Return exact bytes/digest and bounded verification state; no server-side destination registry. |
| Web UI     | Preview compiled context, citations, full token cost, omitted facets and staleness. Export downloads verified bytes to the browser; host-file refresh remains CLI/local-SDK only, with an actionable command recipe.                                                   |
...
- **R1:** [paraphrase] Generate a source-grounded context artifact from a verified Capsule with exact citations, explicit scope and a whole-output token budget. Errors: invalid Capsules, missing provenance, unavailable recorded active tokenizer or framing that cannot fit fail without a misleading partial success.
...
- **R4:** [inferred] Preserve citations, required facet coverage reporting, output ownership and private-source boundaries across CLI/MCP/SDK/REST/Web UI. Errors: remote file-writing requests are not exposed; excluded sources, secret-bearing metadata and recursive generated-context ingestion are refused or explicitly fenced.
...
- [inferred] Maintainability (plan review): duplication - call the existing evidence selector and verifier; the renderer contributes framing overhead and formatting only. Structure - keep file publication local and reuse existing egress lineage instead of adding registries or a sensitivity framework.
