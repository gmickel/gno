...
Add bounded typed metadata extraction, storage and a shared predicate validator/compiler. Support strings, finite numbers, booleans and homogeneous flat arrays of these scalars. Distinguish missing keys from present false/zero/empty values; null and nested arbitrary objects are unsupported in the initial typed namespace. Use an opt-in nested YAML mapping `gno:` containing `metadata:` containing custom keys, not a literal dotted key. Extract that namespace with Bun.YAML and validate the parsed types with the shared bounded validator; leave the existing parser for ordinary tags/metadata unchanged. Null, Date objects, nested maps and mixed arrays are invalid typed values. Record adapters supply `RecordMetadata.custom?: Record<string, TypedValue>` through the same validator, and adapter-fingerprint changes participate in normal re-ingestion; existing ordinary frontmatter and fixed filters continue to behave as before.

Reuse the existing INGEST_VERSION and unchanged-source repair path for extraction upgrades. Per-document ingest-version progress provides resumability; report relevant documents below the required ingest version as incomplete coverage. Do not add a second freshness column, independent backfill runner or competing repair state. Parse changes need not re-embed unchanged text; actual source content changes retain existing embedding identity rules. Invalid opted-in metadata is diagnosed and excluded from typed-filtered retrieval until corrected, while ordinary search remains available. Documents without the namespace have an empty valid metadata map. Invalid namespaces fail a separate eligibility guard before evaluating any predicate, including not and exists:false; negation never makes invalid metadata eligible. Bound key counts, string/array lengths, nesting and total predicate size.

Predicate vocabulary: logical `and`, `or`, `not`; scalar `eq`, `ne`, `gt`, `gte`, `lt`, `lte`; set membership `in`, `nin`, `all`; and `exists`. Define one recursive discriminated contract before implementation. Comparisons are type-strict; numeric ordering never compares number-like strings. Ordered comparisons accept numbers only initially. Text equality is exact and case-sensitive; fixed author/tag behavior remains unchanged. Store the validated typed map in a per-document JSON column and compile predicates into parameterized, two-valued SQL eligibility expressions through the existing buildEligibleDocumentQuery seam. Each leaf explicitly checks presence/type or uses EXISTS, so missing keys yield false before logical not; SQL NULL must not silently replace the declared boolean semantics. Never bulk-materialize every document/chunk to implement this filter. Reuse one normalization contract and shared truth-table fixtures for any SQL/diagnostic evaluator. No custom executable predicates, SQL fragments or regex evaluation.
...
Introduce one `filter` predicate across supported search, vector, hybrid, structured, ask and context-building inputs, using CLI JSON input and typed SDK/MCP/REST schemas. Every backend intersects the predicate with caller authority, collection/path scope, exclusions and managed-memory visibility before candidate limits, fusion, reranking and packing. No selective post-filter approximation that silently loses eligible results. Canonical predicates round-trip through Capsule scope and retrieval-trace filters, hashes, verification and replay. Version the strict Capsule/output contracts compatibly; existing filter-free Capsules continue to verify under their original version, and the new version declares the predicate explicitly. Explain/diagnose exposes bounded invalid/missing/backfill/filter reasons without leaking excluded metadata.
...
| Web UI     | Accessible common field/operator/value controls plus an advanced nested-filter editor; active predicates survive navigation/shareable query state where current privacy policy allows. Show stored typed metadata from the API and validation/backfill warnings; do not infer types again with the browser frontmatter parser. |
...
- [inferred] Maintainability (plan review): duplication - reuse INGEST_VERSION repair rather than inventing metadata freshness state; share predicate normalization and oracle fixtures across storage and diagnosis. Structure - a bounded predicate compiler plugs into existing SQL eligibility without adding unbounded post-filter materialization.
