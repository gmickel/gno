...
**Apply spike findings (fn-72.9):**
- Read fn-72.9's done summary before starting. If the spike identified LLM-thread hang, move the detach-spawn earlier in the command action handler than originally planned.
...
- **Bun gotcha:** pass a numeric fd for stdio redirection to a file; `Bun.file()` objects get closed on parent exit.
- PID reuse mitigation: after `kill(pid, 0)` success, cross-check stored `cmd`/`version` fields.
- Detach must happen early in the action handler — before any LLM port loads, or `cleanupAndExit` may hang on native threads (`src/llm/nodeLlamaCpp/lifecycle.ts`).
## Acceptance
