...
**Size:** M
**Files:** `scripts/package-smoke.ts`, `scripts/package-smoke-mcp.ts`, `scripts/serve-shutdown-smoke.ts`, `test/mcp/http-transport.test.ts`, `test/mcp/http-parity.test.ts`, `.github/workflows/ci.yml`, `docs/MCP.md`, `docs/DAEMON.md`, `docs/API.md`, `docs/ARCHITECTURE.md`, `docs/TROUBLESHOOTING.md`, `assets/skill/SKILL.md`, `assets/skill/mcp-reference.md`, `README.md`, `/Users/gordon/work/gno.sh`
...
- Extend the existing `scripts/package-smoke.ts` and `scripts/package-smoke-mcp.ts` harnesses with the secured `/mcp` gateway coverage, using task 2's `test/mcp/http-transport.test.ts` and `test/mcp/http-parity.test.ts` contracts as the transport baseline.
