...
satisfies: [R1, R2, R3]
---
# fn-152-stop-indexing-polling-after-component.1 Implement indexing polling cleanup on unmount
...
Implementation model: gpt-6-astra medium.
stage: impl-review - skipped(config: user disabled reviews)
stage: plan-sync - skipped(config: disabled)
## Evidence
- Commits:
- Tests: bun test test/serve/public/components/IndexingProgress.dom.test.tsx — 8 pass, 49 assertions, targeted oxlint type-aware/type-check and oxfmt — passed, agent-browser current-source Collections navigation, delayed status, remount — zero requests after cleanup; one 1003/1002ms loop on return; .flow/artifacts/fn-152-stop-indexing-polling-after-component/summary.md
- PRs: