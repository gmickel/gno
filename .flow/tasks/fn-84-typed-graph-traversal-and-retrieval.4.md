...
- Per-stage payload: `present`, `rank`, `score`, `survived`, `dropReason`. Typed metadata reads `contentType`, `content_type_source` (now a persisted column via task .1/.2), categories, fingerprint match/mismatch, **`graphHints`** (the target's content-type hints, surfaced as a real field), plus chunk/line explanation.
