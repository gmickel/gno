...
Follow-up host finding corrected: unscoped mixed-model status accepts fresh legacy coverage only for models without selected or activated variant authority. The regression was observed failing first (expected zero pending, actual three), then passes with scoped filtering, stale legacy timestamps, and revocation when the legacy model gains verified selection. The SDK embed fixture now includes production schema_meta and asserts that verified dry runs record selection. Follow-up focused status/SDK: 9 passed; store/embed/SDK verification: 331 passed; lint: zero errors, 26 inherited warnings.
## Evidence
- Commits: b42a7191ff4223534907b1ed30ad4ee456a69769, 00c777452e5ca344f0f18028fce5c4acf9f4b610, ada250d1a41ab84e46437a84a89f77ff0fa53c09
- Tests: baseline: green — bun test test/store (275 pass); bun run lint:check, bun test test/store test/embed test/cli/embed.test.ts test/cli/status.test.ts test/serve/api-status.test.ts (342 pass, 0 fail), bun test test/store/variant-status.test.ts (6 pass, 0 fail), bun run lint:check (pass; 26 inherited warnings, 0 errors), bun test test/store (post-commit: 281 pass, 0 fail; green receipt b42a7191-unittest), bun test test/store/variant-status.test.ts test/sdk/embed.test.ts (follow-up: 9 pass, 0 fail), bun test test/store test/embed test/sdk/embed.test.ts (follow-up: 331 pass, 0 fail), bun run lint:check (follow-up: pass, 26 inherited warnings, 0 errors)
- PRs: