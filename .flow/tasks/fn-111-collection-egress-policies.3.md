...
**Size:** M
**Files:** `src/serve/server.ts`, `src/mcp/http-transport.ts`, `src/publish/export-service.ts`, `src/llm/httpEmbedding.ts`, `src/llm/httpGeneration.ts`, `test/egress/enforcement.test.ts`
...
- Require all participating collection policies plus transport auth; keep clipper loopback local and public fn-103 projection derived only from explicitly remote-approved publication.
