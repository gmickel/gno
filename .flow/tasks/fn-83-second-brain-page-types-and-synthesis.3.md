...
Make frontmatter `type` the canonical `contentType` — gated on a configured `contentTypes[].id` — via the documented priority chain, additively (existing notes stay valid). Plumb the normalized content-type rules through `SyncOptions` and **every sync entrypoint**, and define a config-change backfill that survives later user edits (not just a one-time version bump). Expose per-result `contentType` consistently in search/query JSON output + schemas. Reads the normalized `contentTypes` rules from `.2` and preset IDs from `.1`.
...
- **Plumb config into ingestion:** add a `contentTypeRules` field to `SyncOptions` (`src/ingestion/types.ts:118`); thread it into `extractDocumentMetadata` (`sync.ts:246`, call site `:565`). Make **every sync entrypoint** pass the rules from `.2`'s exported `normalizeContentTypes()` (the loader result already carries them; in-memory SDK/serve paths call the normalizer directly) or explicitly default to `[]` (MCP sync/index, CLI shared, SDK client, serve background-runtime/watch-service/api). No entrypoint receives raw, un-normalized `contentTypes`.
...
- `src/config/types.ts` `contentTypes` + normalized rules (from `.2`).
