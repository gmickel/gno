...
**Size:** M
**Files:** `src/serve/status.ts`, `src/serve/status-model.ts`, `src/serve/routes/api.ts`, `src/mcp/http-transport.ts`, `src/mcp/http-session.ts`, `src/cli/commands/serve.ts`, `src/cli/commands/daemon.ts`, `src/serve/public/components/HealthCenter.tsx`, `spec/output-schemas/`, `test/mcp/http-transport.test.ts`, `test/mcp/http-parity.test.ts`, `test/serve/resident-concurrency.test.ts`, `test/serve/resident-health.test.ts`
...
- Project the HTTP MCP transport's existing `activeRequests` and `activeSessions` through the resident status model; do not duplicate session accounting outside `HttpMcpSessionStore`.
