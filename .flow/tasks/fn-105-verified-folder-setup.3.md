...
### Approach
- Treat the landed `setup()`/`SetupCommandResult@1.0` from fn-105.2 as the closed standalone lexical-plus-semantic command result. Connector CLI composition runs only after `setup()` returns lexical success; it must not add a connector field to v1.0 or mutate `SetupSemanticReceipt@1.0`. Freeze any required versioned outer JSON contract before implementation and reference the unchanged setup result plus shipped `ActivationVerificationReceipt@1.0` instead of duplicating either schema. <!-- Updated by plan-sync: fn-105.2 closed setup-command-result@1.0 around lexical and semantic only; connector composition is downstream. -->
...
- Preserve fn-105.2 semantic source identity: `setupSemanticSourceFingerprint` is derived from stable lexical setup inputs/fingerprints, collection/path, and activation evidence rather than timestamps, stage tokens, disposition, semantic status, job ID, or PID. Connector execution and onboarding must never key identity or retry decisions from transient semantic worker state.
- Preserve `--no-semantic`/live-owner semantics. A disabled rerun returns semantic `skipped` and starts nothing, but may retain a non-null PID when a still-live one-shot worker already owns the canonical receipt. Do not kill, replace, duplicate, or report that worker as completed; surface skipped intent separately from live process ownership.
...
- Keep `gno setup` direct and standalone. Adding explicit connector verification must not probe, attach to, or enqueue through a resident/MCP/Web runtime; Web/Desktop use their already-owned APIs and lifecycle independently.
...
## Acceptance
- [ ] Connector CLI execution begins only after landed `setup()` returns exit 0 with a completed/ready lexical receipt; `setup-command-result@1.0` and `setup-semantic@1.0` remain closed and unchanged, and any combined JSON contract references rather than copies their schemas plus the shipped connector activation receipt.
- [ ] Connector/onboarding identity is independent of semantic job ID, status, and PID; the stable semantic source fingerprint remains authoritative for semantic reuse, while connector reuse remains activation-fingerprint scoped.
- [ ] A `--no-semantic` rerun never starts or stops semantic work. If the canonical semantic receipt retains a live PID, UI/CLI composition distinguishes skipped request intent from live worker ownership and never claims completion or spawns a duplicate.
