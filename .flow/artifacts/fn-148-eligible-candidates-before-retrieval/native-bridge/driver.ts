...
import { createPinnedSeed } from "./seed";
...
if (process.argv[2] === "prepare") {
  ...
  if (!base) await createPinnedSeed(path, directory);
  const db = new Database(base ?? path, { readonly: true });
  try {
    assertBase(db);
    if (base) db.query("VACUUM INTO ?").run(path);
  } finally {
    db.close();
  }
  ...
  await write("manifest", {
    ...
    seedMode: base
      ? "same-host baseline snapshot clone"
      : "local reconstruction from pinned synthetic text; no index transport",
