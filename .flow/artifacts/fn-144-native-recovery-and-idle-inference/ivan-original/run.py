...
def sha(p):
 ...
 return h.hexdigest()
assert sha(R/'source.tar')=='f5373e3a2d881bca3d4a0ea3c26edb3d4f761529d80f246e0ddbcb59c8fc8743'
...
(E/'pins.json').write_text(json.dumps({'commit':commit,'archiveSha256':sha(R/'source.tar'),'bun':B,'bunSha256':sha(pathlib.Path(B)),'config':config,'configSha256':sha(R/'config/index.yml'),'corpus':fixture,'models':models,'configurationStratum':'explicit default-equivalent; historical exact preset unknown'},indent=2))
