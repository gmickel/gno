...
for(const workload of ["warm30","original143"]){
 const baseline=await Bun.file(`${root}/observations/capture-${workload}-baseline/result.json`).json();
 const candidate=await Bun.file(`${root}/observations/capture-${workload}-candidate/result.json`).json();
