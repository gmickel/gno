import collections, gzip, hashlib, json, math, pathlib, statistics

root=pathlib.Path(__file__).resolve().parent
def relocate(path):
    return pathlib.Path(str(path).replace('/home/gordon/.cache/agent-tmp/gno-fn144-burst-9d0b57e3',str(root)))
def text(path):
    path=relocate(path)
    if path.suffix=='.gz':return gzip.decompress(path.read_bytes()).decode()
...
for workload in ['warm30','original143']:
    references[workload]=load(root/'observations'/f'capture-{workload}-baseline'/'result.json')['results'][0]['raw']
rows=[];missing=[];groups=collections.defaultdict(list);all_calls=[]
for plan_path in load(root/'execution-order.json'):
    plan=load(pathlib.Path(plan_path));directory=relocate(pathlib.Path(plan['outputPath'])).parent
    process=load(directory/'process-receipt.json')
    if not (directory/'result.json').exists() and not (directory/'result.json.gz').exists():
        missing.append({'observation':directory.name,'role':plan['role'],'mode':plan['mode'],'process':process,'declaredCalls':plan['count']+1,'interruptedColdCalls':1,'unstartedBurstCalls':plan['count'],'stderr':text(directory/'stderr.log')})
        continue
...
hash_events=[json.loads(line) for path in (root/'observations/hash-diagnostic-candidate/hash-streams').glob('*.jsonl.gz') for line in text(path).splitlines()]
