...
for plan_path in load(root/'execution-order.json'):
    ...
    if not (directory/'result.json').exists():
        missing.append({'observation':directory.name,'role':plan['role'],'mode':plan['mode'],'process':process,'declaredCalls':plan['count']+1,'interruptedColdCalls':1,'unstartedBurstCalls':plan['count'],'stderr':(directory/'stderr.log').read_text()})
        continue
