...
for index, scenario in enumerate(module.SCENARIOS):
    ...
    results.append({'scenario':scenario['id'], 'agent':f'/root/native_package/clarification_case{index:02}', 'response_sha256': hashlib.sha256(output.encode()).hexdigest(), 'checks': module.run_checks(output,scenario['checks'])})
