...
const activeDecode=(op:string,after:number)=>{
 ...
 return rows.findLast(row=>row.at>=after&&row.event.kind==='evaluation-pending'&&row.event.method==='next'&&row.event.request?.op===op&&!rows.some(end=>end.pid===row.pid&&end.event.id===row.event.id&&end.event.evaluationId===row.event.evaluationId&&['evaluation-end','evaluation-error'].includes(end.event.kind)));
};
try{
 ...
 for(const op of ['rerank','generate']){
  ...
  await poll(()=>Boolean(activeDecode(op,at))||complete);const phase=activeDecode(op,at);const state=observe();const exercised=Boolean(phase&&!complete&&state.busy&&state.pending.some(item=>item.ownsNative));
