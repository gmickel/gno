...
const records=shapes.map(shape=>({telemetry:captureContextArguments([shape]),model:captureArguments(captureContextModelArguments([shape]))}));
...
await Bun.write(`${import.meta.dir}/context-preflight.json`,JSON.stringify({nativeExecution:false,unknownRejected,records},null,2));
