...
**For an authorized product release:** follow [RELEASING.md](RELEASING.md) end
to end: release PR, tag, publish checks, gno.sh propagation (docs, feature
pages, landing-page cards), the post-publish site deploy, and the mickel.tech
GNO page. The downstream sites are part of every release; do them without
being asked.
...
- After merging website changes to `gno.sh` `main`, deploy from `~/work/gno.sh` on a machine whose SSH key the server accepts (currently heimdall). Changes that document unreleased GNO behavior deploy only after that release publishes ([RELEASING.md](RELEASING.md)):

```bash
DEPLOY_HOST=root@178.104.180.89 ./scripts/deploy-prod.sh
```

- The deploy script builds `origin/main` into `/srv/gno-sh/repo/.output`, prerenders, and restarts `gno-sh`; it leaves the remote source checkout alone.
- Verify with `curl -fsSI https://gno.sh`, `ssh root@178.104.180.89 "systemctl is-active gno-sh"`, and confirm `/srv/gno-sh/repo/.output/REVISION` matches `origin/main`.
