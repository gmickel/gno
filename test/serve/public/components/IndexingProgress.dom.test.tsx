...
// Other UI suites replace this component and apiFetch with process-global module
// mocks. Run this real-fetch lifecycle regression in a fresh Bun test process.
if (process.env.GNO_INDEXING_PROGRESS_TEST_CHILD !== "1") {
  test("IndexingProgress lifecycle regressions run against real modules", () => {
    const result = Bun.spawnSync([process.execPath, "test", import.meta.path], {
      env: { ...process.env, GNO_INDEXING_PROGRESS_TEST_CHILD: "1" },
      stdout: "pipe",
      stderr: "pipe",
      timeout: 30_000,
    });
