...
test("opening a pre-feature schema preserves schema identity, chunks and legacy vectors", async () => {
  ...
  const beforeSchema = old
    .query(
      "SELECT name, sql FROM sqlite_master WHERE name NOT IN ('documents', 'idx_documents_active') AND name NOT LIKE '%vector_partitions%' AND name NOT LIKE '%vector_runtime_%' ORDER BY name"
    )
    .all();
  old.close();
  ...
  try {
    ...
    // only drops idx_documents_active (fn-192); chunk policy still performs
    // no schema migration or legacy vector rewrite.
    const columns = db
