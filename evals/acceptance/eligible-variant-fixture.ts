...
export async function setup(size: number, previousDirectory: string) {
  ...
  const actualBase = client.db
    ...
    .all();
  const expectedBase = fixture.map((item) => ({
    id: item.index + 1,
    uri: item.uri,
    hash: item.hash,
    title: "Record",
  ...
  if (canonicalFingerprint(actualBase) !== canonicalFingerprint(expectedBase))
    throw new Error(
      "Legacy snapshot no longer matches pinned base owner/chunk inputs"
    );
